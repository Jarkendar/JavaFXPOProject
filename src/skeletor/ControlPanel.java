...
public class ControlPanel {
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i<10; i++){
            addRandomDeliverer(deliverers_list);
            try{
                sleep(1);
            }catch (InterruptedException e){
                System.out.println(e);
        ...
        displayDeliverers(deliverers_list);
        displayClient(clients_list);
     ...
     */
    private static void displayDeliverers(LinkedList<Deliverer> deliverers){
     ...
     * @param deliverers - lista dostawców
     */
    private static void addRandomDeliverer(LinkedList<Deliverer> deliverers){
     ...
     */
    private static int[] generateHoursWordDeliverer(){
        Random random = new Random(System.currentTimeMillis());
     ...
     */
    private static boolean checkFillHoursWorkArray(int[] hours){
     ...
     */
    private static E_Dni[] generateDaysWorkDeliverer(){
        Random random = new Random(System.currentTimeMillis());
     ...
     */
    private static boolean checkFillDaysWorkArray(E_Dni[] e_dni){
     ...
     */
    private static boolean uniqPESELNumber(LinkedList<Deliverer> deliverers, long newPESEL){
     ...
     * @return
     */
    private static long generatePESELNumber(){
        Random random = new Random(System.currentTimeMillis());
     ...
     */
    private static void displayClient(LinkedList<Client> clients){
     ...
     */
    private static boolean checkUniqREGON(LinkedList<Client> clients, long REGON){
        if (clients.size() != 0) {
            for (Client x : clients) {
     ...
     * @return - true jeśli już wystąpił, false jeśli jest unikalny
     */
    private static boolean checkUniqAccountNumber(LinkedList<Client> clients, String accountNumber){
        if (clients.size()!=0) {
     ...
     * @return - true jeśli już wystąpił, false jeśli jest unikalny
     */
    private static boolean checkUniqClientID(LinkedList<Client> clients, int newClientID){
        if (clients.size() != 0) {
            for (Client x : clients) {
     ...
     */
    private static void addRandomClient(LinkedList<Client> clients_list){
        Random random = new Random(System.currentTimeMillis());
     ...
     * @return - numer regon
     */
    private static long generatorRegonNumber(){
        Random random = new Random(System.currentTimeMillis());
     ...
     */
    private static int getPrivateNumberOfOrdered(){
        int code = 0;
        Random random = new Random(System.currentTimeMillis());
        for (int i =0; i<6; i++){
     ...
     */
    private static String randomStringName(int lenght){
        Random random = new Random(System.currentTimeMillis());
        String string = "";
